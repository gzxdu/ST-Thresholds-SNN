...
class LIFAct(SpikeModule):
    ...
    def __init__(self, step, channel):
        ...
        self.temporal = nn.Parameter(torch.zeros(self.step))
        self.bn = NAM(channel)
    ...
    def forward(self, x):
        ...
        out = []
        self.weight = torch.exp(self.temporal)
        for i in range(self.step):
            u, out_i = mem_update(bn=self.bn,x_in=x[i], mem=u, V_th=self.V_th / self.weight[i],
...
class NAM(nn.Module):
    def __init__(self, channels, t=16):
        ...
        self.my_NAM = nn.BatchNorm2d(self.channels, affine=True)
        nn.init.constant_(self.my_NAM.weight, 1)
        # nn.init.constant_(self.bn2.bias, 0.5)

    def forward(self, x):
        residual = x
        x = self.my_NAM(x)
        weight_bn = self.my_NAM.weight.data.abs() / torch.sum(self.my_NAM.weight.data.abs())
...
class SpikeDropout2d(SpikeModule):
    def __init__(self, dropout: nn.Dropout, step=2):
        super().__init__()
...
class TEBN(SpikeModule):
    def __init__(self, BN: nn.BatchNorm2d, step=2):
        super(TEBN, self).__init__()
        # self.bn = SeqToANNContainer(nn.BatchNorm2d(out_plane))
        self.bn = BN
        self.step = step
        self.p = nn.Parameter(torch.ones(step, 1, 1, 1, 1))
    def forward(self, x):
        T, N, C, H, W = x.shape
        out = self.bn(x.reshape(-1, C, H, W).contiguous())
        out = out.view(T, N, C, H, W)
        out = out * self.p
        return out
...
class myBatchNorm3d(SpikeModule):
    ...
    def forward(self, x):
        # if self._spiking is not True:
        #     return BN(x)
        out = x.permute(1, 2, 0, 3, 4)  # T N C H W ->N C T H W
        out = self.bn(out)
        out = out.permute(2, 0, 1, 3, 4).contiguous()  # N C T H W ->T N C H W
        return out
