...
class SpikingRNNBase(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bias=True, dropout_p=0,
        ...
        * :ref:`API in English <SpikingRNNBase.__init__-en>`
        ...
        :param invariant_dropout_mask: 若为 ``False``，则使用普通的 `Dropout`；若为 ``True``，则使用SNN中特有的，`mask` 不
            随着时间变化的 `Dropout``，参见 :class:`~spikingjelly_data.clock_driven.layer.Dropout`。默认为 ``False``
        ...
        * :ref:`中文API <SpikingRNNBase.__init__-cn>`
        ...
        :param invariant_dropout_mask: If ``False``，use the naive `Dropout`；If ``True``，use the dropout in SNN that
            `mask` doesn't change in different time steps, see :class:`~spikingjelly_data.clock_driven.layer.Dropout` for more
    ...
    def base_cell():
        ...
        * :ref:`API in English <SpikingRNNBase.base_cell-en>`
        ...
        :return: 构成该RNN的基本RNN Cell。例如对于 :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTM`，
            返回的是 :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTMCell`
        :rtype: nn.Module
        ...
        * :ref:`中文API <SpikingRNNBase.base_cell-cn>`
        ...
        :return: The base cell of this RNN. E.g., in :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTM` this function
            will return :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTMCell`
        :rtype: nn.Module
        '''
    ...
    def states_num():
        ...
        * :ref:`API in English <SpikingRNNBase.states_num-en>`
        ...
        :return: 状态变量的数量。例如对于 :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTM`，由于其输出是 ``h`` 和 ``c``，
            因此返回 ``2``；而对于 :class:`~spikingjelly_data.clock_driven.rnn.SpikingGRU`，由于其输出是 ``h``，因此返回 ``1``
        :rtype: int
        ...
        * :ref:`中文API <SpikingRNNBase.states_num-cn>`
        ...
        :return: The states number. E.g., for :class:`~spikingjelly_data.clock_driven.rnn.SpikingLSTM` the output are ``h``
            and ``c``, this function will return ``2``; for :class:`~spikingjelly_data.clock_driven.rnn.SpikingGRU` the output
...
class SpikingLSTMCell(SpikingRNNCellBase):
    def __init__(self, input_size: int, hidden_size: int, bias=True,
        ...
        * :ref:`API in English <SpikingLSTMCell.__init__-en>`
        ...
        :type surrogate_function1: spikingjelly_data.clock_driven.surrogate.SurrogateFunctionBase
        :param surrogate_function2: 反向传播时用来计算脉冲函数梯度的替代函数, 计算 ``g`` 反向传播时使用。 若为 ``None``, 则设置成
            ``surrogate_function1``。默认为 ``None``
        :type surrogate_function2: None or spikingjelly_data.clock_driven.surrogate.SurrogateFunctionBase
